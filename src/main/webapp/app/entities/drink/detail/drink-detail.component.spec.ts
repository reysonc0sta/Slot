...
describe('Drink Management Detail Component', () => {
  ...
  describe('OnInit', () => {
    it('Should load drink on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', DrinkDetailComponent);

      // THEN
      expect(instance.drink()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
